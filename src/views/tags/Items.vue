            ...
            <nav class="level">
              ...
              <div class="level-right">
                <div class="level-item">
                  <div class="buttons">
                    <b-button type="is-link" icon-right="pencil" @click="editTag(item)"></b-button>
                    <b-button type="is-danger" icon-right="delete" @click="deleteTag(item)"></b-button>
                  </div>
                </div>
              </div>
            </nav>
...
export default {
  ...
  created: function () {
    ...
    this.$on('requestForItems', () => {
      ...
      this.$pouchStorage.fetchItemsDB(this.$pouch, 'tags', this.pages.scheme).then((items) => {
        this.items = items
  ...
  methods: {
    ...
    editTag: function (item) {
      this.$buefy.dialog.prompt({
        ...
        trapFocus: true,
        onConfirm: (value) => {
          this.$pouchStorage.editTPFromTasks(this.$pouch, 'tags', item._id, value)
        }
      })
    },
    deleteTag: function (item) {
      this.$buefy.dialog.confirm({
        ...
        hasIcon: true,
        onConfirm: () => {
          this.$pouchStorage.deleteTPFromTasks(this.$pouch, 'tags', item._id)
